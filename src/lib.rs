...
use kzg::{
    ...
    eip_4844::{load_trusted_setup_rust, blob_to_kzg_commitment_raw},
    G1,
};
...
impl KzgWrapper {
    ...
    fn process_single_commitment(&self, blob_bytes: &Uint8Array) -> Result<String> {
        ...
        let commitment = blob_to_kzg_commitment_raw(blob_array, &self.settings)
            .map_err(|e| Error::new(
                Status::GenericFailure,
    ...
    fn process_single_blob_proof(&self, blob_bytes: &Uint8Array) -> Result<Vec<String>> {
        ...
        let (_, proofs): CellsKzgProofs = compute_cells_and_kzg_proofs_raw::<BlstBackend>(
            blob_array,
            &self.settings,
        )
            .map_err(|e| Error::new(
    ...
    fn parse_blob_array(blob_bytes: &Uint8Array) -> Result<[u8; BYTES_PER_BLOB]> {
        ...
        if slice.len() != BYTES_PER_BLOB {
            return Err(Error::new(
                Status::InvalidArg,
                format!("Invalid blob length: expected {} bytes, got {} bytes", BYTES_PER_BLOB, slice.len()),
            ));
        }
